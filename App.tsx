  ...
  const captureAndVerify = async () => {
    ...
    try {
      ...
      if (normalizedResult.includes("YES")) {
        setIsSuccess(true);
        setStatusText(t('Verified Citizen'));
        onSuccess(t("Verified Citizen"));
      } else {
        const reason = result.includes("NO:") ? result.split("NO:")[1].trim() : "Ensure face is clear";
        setError(`${t('Retry')}: ${reason.split('.')[0]}`);
        setStatusText('');
      }
    } catch (err) {
    ...
    <div className="space-y-6 flex flex-col items-center animate-in zoom-in duration-300">
      ...
      {!isSuccess && (
        <div className="flex gap-3 w-full">
           <Button onClick={captureAndVerify} isLoading={loading} disabled={loading} className="flex-1 bg-indigo-600 hover:bg-indigo-700 py-4 font-black text-[10px] uppercase tracking-widest shadow-xl">{t("Scan")}</Button>
           <Button variant="outline" onClick={stopCamera} className="flex-1 py-4 font-black text-[10px] uppercase tracking-widest border-gray-200">{t("Cancel")}</Button>
        </div>
      )}
...
export default function App() {
  ...
  const [marketItems, setMarketItems] = useState<MarketItem[]>([
    { id: '1', name: 'Premium Basmati Rice', price: '₹75/kg', seller: 'Ramesh Farmer', location: 'Nagpur', image: 'https://images.unsplash.com/photo-1586201375761-83865001e31c?auto=format&fit=crop&q=80&w=400', contact: '9876543210' },
    { id: '2', name: 'Fresh Organic Wheat', price: '₹40/kg', seller: 'Suresh Agriculture', location: 'Wardha', image: 'https://images.unsplash.com/photo-1574323347407-f5e1ad6d020b?auto=format&fit=crop&q=80&w=400', contact: '9123456789' },
    { id: '3', name: 'Desi Ghee', price: '₹600/L', seller: 'Gokul Dairy', location: 'Amravati', image: 'https://images.unsplash.com/photo-1589927986089-35812388d1f4?auto=format&fit=crop&q=80&w=400', contact: '9000011122' }
  ]);
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('grameen_user');
    if (savedUser) {
      try {
        ...
        localStorage.removeItem('grameen_user');
      }
    }
  }, []);
  ...
  const handleLogin = async (faceAuthName?: string) => {
    if (faceAuthName) {
      const newUser = { name: faceAuthName, email: 'face-auth@grameen.com' };
      setUser(newUser);
      localStorage.setItem('grameen_user', JSON.stringify(newUser));
      setModal(ModalType.NONE);
      setView(AppView.PORTAL);
      speak(t("Welcome") + " " + faceAuthName, language);
      return;
    }
    ...
    // Mock Login Logic
    setTimeout(() => {
      const newUser = { name: email.split('@')[0] || 'User', email: email };
      setUser(newUser);
      localStorage.setItem('grameen_user', JSON.stringify(newUser));
  ...
  const handleSahayakAction = (intent: any) => {
    const targetMap: Record<string, ModalType> = {
      ...
      'governance_aid': ModalType.GOVERNANCE
    };

    if (intent.target === 'plant_health_ai') {
      handleOpenPlantAI();
      return;
    }
  ...
  return (
    <AppContext.Provider value={{ elderMode, toggleElderMode, isOffline, toggleOffline }}>
      <div className={`min-h-screen flex flex-col bg-white font-sans ${elderMode ? 'text-lg' : 'text-base'}`}>
        ...
        <Modal isOpen={modal === ModalType.LOGIN} onClose={() => { setModal(ModalType.NONE); setLoginError(null); }} title={t("Welcome")} maxWidth="max-w-md">
          <div className="space-y-6">
            ...
            {loginError && <p className="text-red-500 text-xs font-bold uppercase tracking-tight">{loginError}</p>}
            <Button onClick={() => handleLogin()} isLoading={loginLoading} className="w-full py-4 rounded-xl font-black uppercase tracking-widest">{t("Sign In / Join")}</Button>
            <div className="relative flex justify-center text-[10px] font-black uppercase tracking-[0.4em] text-gray-300 py-2">
              ...
              <span className="bg-white px-4 relative">{t("OR")}</span>
            </div>
