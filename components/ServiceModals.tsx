  ...
  const handleSend = async (forcedText?: string) => {
    ...
    try {
      ...
      if (sahayakResult && sahayakResult.action !== "unknown") {
        ...
        const text = sahayakResult.text || `I can help with: Kisan Mandi, Swasthya Saathi, Resume, and Mobility Planner. Just tell me what you need.`;
       ...
       <div className="flex flex-col h-[550px]">
         ...
         <div className="pt-4 mt-4 border-t flex flex-col gap-3">
           <div className="flex gap-2">
             ...
             <button onClick={startListening} className={`p-3 rounded-full shadow-lg transition-all active:scale-90 ${listening ? 'bg-red-500 animate-pulse text-white' : 'bg-pink-600 text-white hover:bg-pink-700'}`}>
               <Mic size={20} />
             </button>
           </div>
         </div>
...
export const KisanModal: React.FC<{ 
  ...
  const handlePost = async () => {
    ...
    setIsPosting(true);
    // Simulating post logic with local state
    setTimeout(() => {
        const itemToAdd: MarketItem = {
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
          onClick={() => {
            ...
            else { setView('buy'); setSellStep(0); }
          }} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item) => (
            <div key={item.id} className="bg-white border border-gray-100 rounded-[2rem] overflow-hidden hover:shadow-xl transition-all relative group shadow-sm">
              ...
              <button onClick={() => speakItem(item)} className="absolute top-4 right-4 bg-white/95 p-3 rounded-full text-green-600 hover:bg-green-600 hover:text-white transition-all shadow-xl border border-white active:scale-90">
                <Volume2 size={20} />
              </button>
              <div className="p-6">
        ...
        <div className="max-w-2xl mx-auto space-y-6 pb-10">
          {sellStep === 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-bottom-6">
              <button onClick={handleVoiceFill} className="bg-green-600 p-12 rounded-[3rem] border-4 border-white shadow-2xl hover:scale-105 active:scale-95 transition-all flex flex-col items-center gap-6 group">
              ...
              <button onClick={() => setSellStep(1)} className="bg-white p-12 rounded-[3rem] border-4 border-gray-50 shadow-xl hover:scale-105 active:scale-95 transition-all flex flex-col items-center gap-6 group">
